...
namespace Iksula\Storemanager\Block\Role\Tab\Edit;
 ...
 *
 */
class Roleresource extends \Magento\Backend\Block\Widget\Form implements \Magento\Backend\Block\Widget\Tab\TabInterface
{
    /**
     * @var string
     */
    ...
    public function getSelectedResources()
    {
        ...
        if (empty($selectedResources)) {
            ...
            if (null === $selectedResources) {                
                $storemanager_id = $this->_request->getParam('storemanager_id' , false);
                $rid = $this->storemanagerfactory->create()->load($storemanager_id)->getRoleIdMapping(); // Get Role id by store manager.
