...
class Orderitemsstatus extends Action
{
    ...
    public function execute()
    {
        ...
        $orderSplitCollection = $this->ordersplitFactory->create()->getCollection()
                            ->addFieldToFilter('order_item_status' , array('eq' => 'unallocated'))
                            ->getData();

        foreach($orderSplitCollection as $ordersplitData){

            $orderData = $this->orderFactoryData->create()->load($ordersplitData['order_id']);
            $aOrderSkuWiseData = json_decode($ordersplitData['order_item_data']);

            $orderShippingAddress = $orderData->getShippingAddress();
            $orderregionid = $orderShippingAddress->getRegionId();
            $this->allocateOrderNearestWarehouse($orderregionid , $aOrderSkuWiseData);
        ...
        $this->storeinventoryAllocationOrderItemStatusMail($time);

    }

     public function storeinventoryAllocationOrderItemStatusMail($time)
    {   
            ...
            $transport = $this->_transportBuilder->setTemplateIdentifier('storeinventory_mail_template')
                ...
                ->setTemplateVars([
                    'message'     => "Storeinventory Order Item Status Cron Run Time",
                    'time'      => $time,
                    ])
                ->setFrom('general')
                ->addTo($emailids)
