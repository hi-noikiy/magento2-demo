    ...
    protected function _prepareForm()
    {
        ...
        $elements['image'] = $fieldset->addField(
            ...
            [
                ...
                'note' => 'Allow image type: (jpg, jpeg, gif, png) and Allowed Dimension : (1100 X 520)',
            ]
        );
...
//            $datetime = new \DateTime($dataObj->getData('end_time'));
        ...
        $Lastfield = $form->getElement('target');
       $Lastfield->setAfterElementHtml(
                '<script> 
                        require(["jquery"], function($){
                            $(document).ready(function(){
                                  
                                var image_obj = $("#page_image_image");
                                if(typeof image_obj.attr("src") === "undefined"){
                                    var page_obj = $("#page_image"); 
                                    image_name = page_obj.attr("name");                                
                                    if(image_name == "image"){
                                        page_obj.addClass("required-entry");
                                    }    
                                }  

                                $("#edit_form").submit( function(submitEvent) { 
                                        var image_obj = $("#page_image_image");

                                    if($("#page_image").val() != "" ){                                       
                                            var ext = $("#page_image").val().split(".").pop().toLowerCase();
                                            if($.inArray(ext, ["gif","png","jpg","jpeg"]) == -1) {
                                                alert("Please upload image file in Banner Image field (pg, jpeg, gif, png)");
                                                e.preventDefault();                                                    
                                            }
                                    }
                                });
